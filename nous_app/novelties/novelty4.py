...
def make_plot(df_single_lines):
    ...
    for i in range(1,39):
        current_lowest_winning_score.append(df[df['gw'] == i]['score'].min())
...
#    rnd = np.searchsorted(current_lowest_winning_score, current_lowest_winning_score.min()).item() + 1
    ...
    nov_4_text = "Current lowest winning score is from gameweek " + str(rnd) + ", when " + team + " (" + owner + ") scored " + str(int(score)) + " points"
    ...
    for owner in set(df['owner']):
        ...
        p.circle('gw', 'score', source=src, alpha='fill_alpha', size=13, color = 'owner_color', 
                 legend_label = owner, muted_color='colors', muted_alpha=0.4, name=owner)
